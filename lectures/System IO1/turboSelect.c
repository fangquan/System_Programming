...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
		fprintf(stderr, "Usage: turboSelect nbytes\n");
		exit(1);
	}
	nbytes = atoi(argv[1]);
...
void relay(int leftwardIn, int leftwardOut, int rightwardIn, int rightwardOut) {
	...
	pc_t *LR = (pc_t *)malloc(sizeof(pc_t));
	pc_t *RL = (pc_t *)malloc(sizeof(pc_t));

	pc_init(LR, leftwardIn, leftwardOut);
	pc_init(RL, rightwardIn, rightwardOut);

	while (!LR->done || !RL->done) {
		// event loop
		int leftreadok = 0;
		int rightreadok = 0;
		int leftwriteok = 0;
		...
		} while ((leftreadok && pready(LR)) || 
				(rightwriteok && cready(LR)) ||
				(rightreadok && pready(RL)) ||
				(leftwriteok && cready(RL)));
		leftreadok = 0;
		rightreadok = 0;
		leftwriteok = 0;
		rightwriteok = 0;
