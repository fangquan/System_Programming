...
static trace_t *read_trace(char *tracedir, char *filename) {
    ...
    strncpy(trace->trace_name, filename, 1024);
...
static void printresults(int n, stats_t *stats) {
    ...
    printf("%6s %4s               %7s %5s%5s%8s%9s\n", 
	   "trace#", " name", " valid", "util", "ops", "secs", "Kops");
    printf("----------------------------------------------------------------\n");
    for (i=0; i < n; i++) {
	if (stats[i].valid) {
	    printf(" %-2d     %-18s   %-4s%5.0f%%%8.0f%10.6f%6.0f\n", 
		   i,
		   stats[i].trace_name,
		   "yes",
		   stats[i].util*100.0,
    ...
    if (errors == 0) {
	printf("%24s%5.0f%%%8.0f%10.6f%6.0f\n", 
	       "Total                            ",
	       (util/n)*100.0,
	       ops, 
	       secs,
